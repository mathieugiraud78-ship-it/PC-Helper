...
if BG_PATH.is_file():
    try:
        ...
        _bg_photo = ImageTk.PhotoImage(_bg_original.resize((1180, 760), Image.Resampling.LANCZOS))
...
# ---------- shell ----------
sidebar = ctk.CTkFrame(app, width=236, corner_radius=0, fg_color=GLASS, border_width=1, border_color=BORDER)
...
def show_home():
    ...
    head.pack(fill="x", padx=30, pady=(26, 6))
    left = ctk.CTkFrame(head, fg_color="transparent"); left.pack(side="left")
    ctk.CTkLabel(left, text="Bienvenue sur PC Helper", text_color=TEXT, font=("Arial", 29, "bold")).pack(anchor="w")
    ctk.CTkLabel(left, text="Surveillez, entretenez et optimisez votre PC.", text_color=MUTED, font=("Arial", 13)).pack(anchor="w", pady=(2,0))
    ctk.CTkLabel(head, text=datetime.now().strftime("%A %d %B %Y  •  %H:%M"), text_color=MUTED, font=("Arial", 10)).pack(side="right", anchor="n", pady=8)

    cards = ctk.CTkFrame(content, fg_color="transparent")
    cards.pack(fill="x", padx=30, pady=14)
    ...
    for i,(icon,title,key) in enumerate(stats):
        card=glass_card(cards, 16)
        card.grid(row=0,column=i,padx=6,sticky="nsew")
        ctk.CTkLabel(card,text=icon,text_color=ACCENT,font=("Arial",22,"bold")).pack(anchor="w",padx=15,pady=(13,0))
        ctk.CTkLabel(card,text=title,text_color=MUTED,font=("Arial",9,"bold")).pack(anchor="w",padx=15,pady=(3,0))
        stat_labels[key]=ctk.CTkLabel(card,text="--",text_color=TEXT,font=("Arial",20,"bold"))
        stat_labels[key].pack(anchor="w",padx=15,pady=(3,13))
    ...
    state=glass_card(lower,20); state.grid(row=0,column=0,sticky="nsew",padx=(0,8))
    ctk.CTkLabel(state,text="État général du PC",text_color=TEXT,font=("Arial",20,"bold")).pack(anchor="w",padx=22,pady=(20,5))
    score_label=ctk.CTkLabel(state,text="",text_color=SUCCESS,font=("Arial",16,"bold")); score_label.pack(anchor="w",padx=22,pady=5)
    details=ctk.CTkLabel(state,text="Analyse en cours…",text_color=MUTED,font=("Arial",13),justify="left",anchor="w"); details.pack(anchor="w",padx=22,pady=8)
    ctk.CTkButton(state,text="🔍  Lancer un diagnostic",height=44,corner_radius=11,fg_color="#263650",hover_color="#334766",command=lancer_diagnostic).pack(anchor="w",padx=22,pady=(15,8))
    ctk.CTkButton(state,text="🚀  Optimiser la RAM",height=44,corner_radius=11,fg_color=ACCENT_2,hover_color="#8B5CF6",text_color="#0B1020",command=optimize_ram).pack(anchor="w",padx=22,pady=8)

    quick=glass_card(lower,20); quick.grid(row=0,column=1,sticky="nsew",padx=(8,0))
    ctk.CTkLabel(quick,text="Actions rapides",text_color=TEXT,font=("Arial",20,"bold")).pack(anchor="w",padx=22,pady=(20,12))
    for text,cmd in [("🧹  Nettoyage rapide",lancer_nettoyage),("💻  Informations système",afficher_infos_pc),("📄  Rapport complet",open_full_report),("📊  Surveillance temps réel",open_realtime_monitor)]:
        ctk.CTkButton(quick,text=text,height=40,corner_radius=10,fg_color="transparent",border_width=1,border_color=BORDER,hover_color="#243149",anchor="w",command=cmd).pack(fill="x",padx=22,pady=5)

    def refresh_home():
        if not app.winfo_exists(): return
        cpu=psutil.cpu_percent(interval=None); ram=psutil.virtual_memory(); disk=psutil.disk_usage(system_drive()); net=psutil.net_io_counters(); score=calculate_health_score()
        stat_labels["cpu"].configure(text=f"{cpu:.0f} %"); stat_labels["ram"].configure(text=f"{ram.percent:.0f} %"); stat_labels["disk"].configure(text=f"{go(disk.free):.0f} Go libres"); stat_labels["net"].configure(text="Connecté" if net else "Inactif")
        score_label.configure(text=(f"●  Tout est en bon état !   {score}/100" if score>=85 else f"●  À surveiller   {score}/100" if score>=65 else f"●  Attention requise   {score}/100"))
        score_label.configure(text_color=SUCCESS if score>=85 else "#FBBF24" if score>=65 else "#FB7185")
        details.configure(text=f"CPU : {cpu:.0f} %    •    RAM : {ram.percent:.0f} %\nDisque : {go(disk.free):.1f} Go libres    •    Uptime : {fmt_duration(time.time()-psutil.boot_time())}")

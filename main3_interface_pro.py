...
# ============================================================
# Modern Main UI — Glass / Professional edition
# ============================================================

app = ctk.CTk()
app.title(f"{APP_NAME} — Créé par {APP_AUTHOR}")
app.geometry("1220x780")
app.minsize(1080, 700)

# Palette « verre » : panneaux sombres, bordures discrètes et accent bleu.
GLASS = "#111827"
GLASS_2 = "#0f172a"
PANEL = "#151d2e"
PANEL_2 = "#192337"
...
BG_CANDIDATES = (BASE_DIR / "pc_background.png", BASE_DIR / "pc_background.jpg.png")
BG_PATH = next((p for p in BG_CANDIDATES if p.is_file()), None)
_bg_original = None
_bg_photo = None
bg_label = None
...
if BG_PATH is not None:
    try:
        ...
        _bg_photo = ImageTk.PhotoImage(_bg_original.resize((1220, 780), Image.Resampling.LANCZOS))
        ...
        def _resize_background(event):
            global _bg_photo
            if event.widget is app and _bg_original is not None and event.width > 0 and event.height > 0:
                resized = _bg_original.resize((event.width, event.height), Image.Resampling.LANCZOS)
...
# Une légère transparence globale donne un rendu plus intégré au bureau Windows.
try:
...
SIDEBAR = 238
sidebar = ctk.CTkFrame(app, width=SIDEBAR, corner_radius=0, fg_color=GLASS)
sidebar.pack(side="left", fill="y")
sidebar.pack_propagate(False)

content = ctk.CTkFrame(app, corner_radius=0, fg_color="#0b1020")
content.pack(side="right", fill="both", expand=True)

# ---------- en-tête de la barre latérale ----------
brand = ctk.CTkFrame(sidebar, fg_color="transparent")
brand.pack(fill="x", padx=18, pady=(24, 8))
ctk.CTkLabel(brand, text="◈", text_color=ACCENT, font=("Arial", 31, "bold")).pack(side="left")
brand_text = ctk.CTkFrame(brand, fg_color="transparent")
brand_text.pack(side="left", padx=10)
ctk.CTkLabel(brand_text, text="PC", text_color=TEXT, font=("Arial", 24, "bold"), anchor="w").pack(anchor="w")
ctk.CTkLabel(brand_text, text="HELPER", text_color=ACCENT, font=("Arial", 13, "bold"), anchor="w").pack(anchor="w")
ctk.CTkLabel(sidebar, text=f"CENTRE DE MAINTENANCE  •  V{APP_VERSION}", text_color=MUTED,
            font=("Arial", 9, "bold")).pack(anchor="w", padx=22, pady=(0, 18))
...
def set_active(label):
    for name, btn in nav_buttons.items():
        btn.configure(fg_color=ACCENT if name == label else "transparent",
                      hover_color=ACCENT_HOVER if name == label else PANEL_2)


def nav_button(label, command):
    btn = ctk.CTkButton(
        sidebar, text=label, command=lambda: (set_active(label), command()),
        height=44, corner_radius=11, anchor="w", font=("Arial", 13, "bold"),
        fg_color="transparent", hover_color=PANEL_2, text_color=TEXT,
        border_width=0
    )
    btn.pack(fill="x", padx=13, pady=4)
    nav_buttons[label] = btn
    return btn

# ---------- petites briques visuelles ----------
def glass_card(parent, **kwargs):
    return ctk.CTkFrame(parent, corner_radius=18, fg_color=kwargs.pop("fg_color", PANEL),
                        border_width=1, border_color=BORDER, **kwargs)


def section_title(parent, title, subtitle=None):
    head = ctk.CTkFrame(parent, fg_color="transparent")
    head.pack(fill="x", padx=32, pady=(26, 12))
    ctk.CTkLabel(head, text=title, text_color=TEXT, font=("Arial", 28, "bold")).pack(anchor="w")
    if subtitle:
        ctk.CTkLabel(head, text=subtitle, text_color=MUTED, font=("Arial", 13)).pack(anchor="w", pady=(3, 0))


def optimize_ram():
    """Libère la partie non essentielle des working sets Windows sans fermer les applications."""
    ...
    try:
        ...
        popup("🚀 Optimisation RAM", "L'API Windows d'optimisation mémoire n'est pas disponible sur ce système.", 650, 400)
        return
    ...
    freed = max(after.available - before.available, 0) / (1024 ** 2)
    log_action("Optimisation RAM", f"processus traités={trimmed}, refusés={skipped}, variation disponible={freed:.1f} Mo")
    popup(
        "🚀 Optimisation RAM",
        "=== OPTIMISATION MÉMOIRE ===\n\n"
        f"RAM disponible avant : {before.available / 1024**3:.2f} Go\n"
        f"RAM disponible après : {after.available / 1024**3:.2f} Go\n"
        f"Variation mesurée : {freed:.1f} Mo\n\n"
        f"Processus traités : {trimmed}\n"
        f"Processus protégés/refusés : {skipped}\n\n"
        "Aucune application n'a été fermée. Windows peut réutiliser cette mémoire immédiatement si nécessaire.",
        680, 500
    )
...
def show_home():
    ...
    left.pack(side="left")
    ctk.CTkLabel(left, text="Bonjour Mathieu 👋", text_color=TEXT, font=("Arial", 28, "bold")).pack(anchor="w")
    ctk.CTkLabel(left, text="Centre de contrôle et d'optimisation de votre PC", text_color=MUTED,
                 font=("Arial", 13)).pack(anchor="w", pady=(3, 0))
    ctk.CTkLabel(header, text="PC HELPER", text_color=ACCENT, font=("Arial", 12, "bold")).pack(side="right", pady=12)

    cards = ctk.CTkFrame(content, fg_color="transparent")
    cards.pack(fill="x", padx=25, pady=12)
    for i in range(4):
    ...
    for i, (icon, title, key) in enumerate(stats):
        card = glass_card(cards)
        card.grid(row=0, column=i, padx=7, sticky="nsew")
        ctk.CTkLabel(card, text=icon, text_color=ACCENT, font=("Arial", 22, "bold")).pack(anchor="w", padx=17, pady=(13, 2))
        ctk.CTkLabel(card, text=title, text_color=MUTED, font=("Arial", 10, "bold")).pack(anchor="w", padx=17)
        stat_labels[key] = ctk.CTkLabel(card, text="--", text_color=TEXT, font=("Arial", 20, "bold"))
        stat_labels[key].pack(anchor="w", padx=17, pady=(3, 14))

    lower = ctk.CTkFrame(content, fg_color="transparent")
    lower.pack(fill="both", expand=True, padx=25, pady=(4, 20))
    lower.grid_columnconfigure(0, weight=3)
    lower.grid_columnconfigure(1, weight=2)
    lower.grid_rowconfigure(0, weight=1)

    state_card = glass_card(lower)
    state_card.grid(row=0, column=0, sticky="nsew", padx=(0, 8))
    ctk.CTkLabel(state_card, text="État général du PC", text_color=TEXT, font=("Arial", 20, "bold")).pack(anchor="w", padx=22, pady=(22, 5))
    score_label = ctk.CTkLabel(state_card, text="Analyse en cours…", text_color=ACCENT, font=("Arial", 18, "bold"))
    score_label.pack(anchor="w", padx=22, pady=7)
    details = ctk.CTkLabel(state_card, text="", text_color=MUTED, font=("Arial", 13), justify="left", anchor="w")
    details.pack(anchor="w", padx=22, pady=7)
    ctk.CTkButton(state_card, text="🔍  Lancer un diagnostic", height=46, corner_radius=12,
                  fg_color=ACCENT, hover_color=ACCENT_HOVER, command=lancer_diagnostic).pack(anchor="w", padx=22, pady=18)

    quick = glass_card(lower)
    quick.grid(row=0, column=1, sticky="nsew", padx=(8, 0))
    ctk.CTkLabel(quick, text="Actions rapides", text_color=TEXT, font=("Arial", 20, "bold")).pack(anchor="w", padx=22, pady=(22, 12))
    quick_items = [
        ("🚀  Optimiser la RAM", optimize_ram),
        ("🧹  Nettoyage rapide", lancer_nettoyage),
        ("📊  Surveillance temps réel", open_realtime_monitor),
        ("📄  Rapport complet", open_full_report),
    ]
    for text, command in quick_items:
        ctk.CTkButton(quick, text=text, height=40, corner_radius=10, fg_color="transparent",
                      border_width=1, border_color=BORDER, hover_color=PANEL_2,
                      anchor="w", command=command).pack(fill="x", padx=20, pady=5)

    footer = ctk.CTkLabel(content, text=f"© 2026 {APP_AUTHOR}  •  Assistant de maintenance informatique", text_color="#8290a8", font=("Arial", 10))
    ...
    def refresh_home():
        ...
        net = psutil.net_io_counters()
        score = calculate_health_score()
        stat_labels["cpu"].configure(text=f"{cpu:.0f} %")
        stat_labels["ram"].configure(text=f"{ram.percent:.0f} %")
        stat_labels["disk"].configure(text=f"{go(disk.free):.0f} Go libres")
        stat_labels["net"].configure(text="Connecté" if net else "Inactif")
        score_label.configure(text=(f"🟢  Système sain  •  {score}/100" if score >= 85 else
                                    f"🟠  À surveiller  •  {score}/100" if score >= 65 else
                                    f"🔴  Attention requise  •  {score}/100"))
        details.configure(text=(f"CPU : {cpu:.0f} %     •     RAM : {ram.percent:.0f} %\n"
                                f"Disque : {go(disk.free):.1f} Go libres     •     Uptime : {fmt_duration(time.time() - psutil.boot_time())}"))
        app.after(1500, refresh_home)
    refresh_home()


def show_monitor_menu():
    show_menu("🖥️  Surveillance", "Surveillez les performances et l'état matériel de votre PC", [
        ("PERFORMANCES", [("📈  Tableau de bord", open_dashboard), ("📊  Moniteur temps réel", open_realtime_monitor), ("🚀  Optimisation RAM", optimize_ram)]),
        ("MATÉRIEL", [("💻  Informations du PC", afficher_infos_pc), ("🌡️  Températures", open_temperatures), ("🔋  Batterie", open_battery), ("🧩  Carte matériel", open_hardware), ("❤️  Score de santé", lambda: popup("❤️ Score de santé", f"Score indicatif : {calculate_health_score()}/100", 500, 330))])
    ])


def show_menu(title, subtitle, groups):
    clear_content()
    section_title(content, title, subtitle)
    box = ctk.CTkScrollableFrame(content, fg_color="transparent")
    box.pack(fill="both", expand=True, padx=25, pady=(0, 22))
    for group_title, buttons in groups:
        ctk.CTkLabel(box, text=group_title, text_color=ACCENT, font=("Arial", 11, "bold")).pack(anchor="w", padx=8, pady=(8, 7))
        panel = glass_card(box)
        panel.pack(fill="x", pady=(0, 16))
        for i, (txt, cmd) in enumerate(buttons):
            panel.grid_columnconfigure(i % 2, weight=1)
            ctk.CTkButton(panel, text=txt, command=cmd, height=48, corner_radius=10,
                          fg_color="transparent", border_width=1, border_color=BORDER,
                          hover_color=PANEL_2, anchor="w").grid(row=i // 2, column=i % 2,
                                                                  padx=10, pady=9, sticky="ew")


# ---------- navigation ----------
nav_button("⌂  Accueil", show_home)
nav_button("🖥  Surveillance", show_monitor_menu)
nav_button("💾  Stockage", lambda: show_menu("💾  Stockage", "Analyse, nettoyage et gestion de l'espace disque", [
    ("STOCKAGE", [("💾  Analyse du disque", open_storage_analysis), ("📦  Gros fichiers", open_large_files), ("🔁  Doublons", open_duplicates), ("🧹  Nettoyage avancé", open_advanced_cleanup), ("💿  État des disques", open_disk_state)])]))
nav_button("⌁  Réseau", lambda: show_menu("🌐  Réseau", "Diagnostic de la connexion et des interfaces réseau", [
    ("RÉSEAU", [("🌐  Test Internet", open_network_test), ("📡  Informations réseau", open_network_info), ("🧭  Test DNS", open_dns_test), ("📡  Scanner LAN", open_lan_scanner), ("📚  Historique réseau", open_connection_history)])]))
nav_button("🛠  Technicien", lambda: show_menu("🛠️  Technicien", "Outils avancés pour le dépannage et la maintenance Windows", [
    ("WINDOWS", [("🛠️  SFC / DISM", open_sfc_dism), ("💿  CHKDSK", open_chkdsk), ("🚀  Démarrage", open_startup), ("⚙️  Services", open_services), ("📊  Processus", open_processes), ("🧩  Pilotes", open_drivers), ("🔄  Mises à jour", open_windows_updates)])]))
nav_button("🛡  Sécurité", lambda: show_menu("🔐  Sécurité", "Vérifiez les protections essentielles de Windows", [
    ("PROTECTION", [("🧱  Pare-feu", open_firewall), ("🛡️  Defender", open_defender), ("🔐  Mises à jour sécurité", open_security_updates), ("🔎  Audit sécurité", open_security_audit), ("🛡️  Score /100", open_security_score)])]))
nav_button("📄  Rapports", lambda: show_menu("📄  Rapports", "Historique, export et comparaison des interventions", [
    ("RAPPORTS", [("📄  Rapport complet", open_full_report), ("📤  Export TXT / CSV / HTML", export_report), ("📚  Historique", open_history), ("↔️  Avant / Après", open_compare_before_after)])]))
nav_button("🧰  Utilitaires", lambda: show_menu("🧰  Utilitaires", "Outils complémentaires de PC Helper", [
    ("OUTILS", [("🔧  Mode Technicien", open_technician_mode), ("🧰  Mode Dépannage", open_troubleshooter), ("☑️  Checklist", open_checklist), ("⏱️  Minuteur", open_timer), ("🌗  Thème clair / sombre", toggle_theme), ("ℹ️  À propos", open_about)])]))

ctk.CTkLabel(sidebar, text="Créé avec passion par Mathieu\nMaintenance • Diagnostic • Optimisation",
            text_color="#8290a8", font=("Arial", 9), wraplength=190, justify="left").pack(side="bottom", anchor="w", padx=22, pady=18)

...
def open_drivers():
    # Get-PnpDevice dépend du module PnpDevice, qui peut être indisponible
    # selon l'édition/version de Windows. On utilise donc CIM en secours.
    code, out, err = run_powershell(
        ...
        "else { throw 'Get-PnpDevice indisponible' }"
    )
    ...
    if not out:
        code, out, err = run_powershell(
            ...
            "Sort-Object Status,PNPClass,Name | "
            "Format-Table -AutoSize | Out-String"
        )

    if not out:
        details = err or "Code de sortie : %s" % code
        popup("🧩 Gestionnaire de pilotes",
              "Impossible de récupérer les périphériques.\n\n" + details, 1200, 700)
        return

    popup("🧩 Gestionnaire de pilotes", out, 1200, 700)
...
# ============================================================
# Modern Main UI — Inspired by the promotional design
# ============================================================
...
# ---------- background ----------
# Works both when running main.py and when packaged with PyInstaller.
BASE_DIR = Path(getattr(sys, "_MEIPASS", Path(__file__).resolve().parent))

# The original image was saved as "pc_background.jpg.png".
# We accept both names so the program works in development and after packaging.
BG_CANDIDATES = (
    "pc_background.png",
    "pc_background.jpg.png",
)
BG_PATH = next(
    (BASE_DIR / name for name in BG_CANDIDATES if (BASE_DIR / name).exists()),
    BASE_DIR / "pc_background.png",
)

try:
    bg_pil = Image.open(BG_PATH).convert("RGB")
    bg_image = ctk.CTkImage(light_image=bg_pil, dark_image=bg_pil, size=(1180, 760))
    bg_label = ctk.CTkLabel(app, text="", image=bg_image)
    bg_label.place(relx=0, rely=0, relwidth=1, relheight=1)
    bg_label.lower()

    def _resize_background(event):
        if event.widget is app and event.width > 0 and event.height > 0:
...
SIDEBAR = 220
sidebar = ctk.CTkFrame(app, width=SIDEBAR, corner_radius=0)
...
ctk.CTkLabel(brand, text="🖥", font=("Arial", 30)).pack(side="left")
brand_text = ctk.CTkFrame(brand, fg_color="transparent")
brand_text.pack(side="left", padx=8)
ctk.CTkLabel(brand_text, text="PC", font=("Arial", 23, "bold"), anchor="w").pack(anchor="w")
ctk.CTkLabel(brand_text, text="Helper", font=("Arial", 18, "bold"), anchor="w").pack(anchor="w")
ctk.CTkLabel(sidebar, text=f"V{APP_VERSION}", font=("Arial", 11)).pack(anchor="w", padx=22, pady=(0, 18))
...
def nav_button(label, command):
    btn = ctk.CTkButton(
        ...
        hover_color=("#d9e6ff", "#173a63"),
        text_color=("#1a1a1a", "#f2f2f2"),
    )
...
def afficher_infos_pc():
    ...
    text = (
        "💻 INFORMATIONS DU PC\n\n"
        f"Système : {info['system']}\n"
        f"Version : {info['version']}\n"
        f"Nom du PC : {info['hostname']}\n"
        f"Processeur : {info['cpu']}\n"
        f"Cœurs : {info['physical_cores']} physiques / {info['logical_cores']} logiques\n"
        f"Architecture : {info['arch']}\n"
        f"RAM : {info['ram_used']:.1f} / {info['ram_total']:.1f} Go ({info['ram_percent']:.0f} %)\n"
        f"Disque : {info['disk_used']:.1f} / {info['disk_total']:.1f} Go\n"
    ...
    popup("💻 Informations du PC", text, 700, 520)
...
def lancer_diagnostic():
    ...
    lines += [
        ...
        f"Disque utilisé : {disk.percent:.0f} %",
        f"Disque libre : {go(disk.free):.1f} Go",
        ""
    ]

    ok, latency = ping_host("1.1.1.1")
    lines.append(f"Internet : ✅ OK ({latency:.1f} ms)" if ok else "Internet : ❌ Échec")

    score = calculate_health_score()
    lines += ["", f"Score de santé : {score}/100"]

    if score >= 85:
        lines.append("État : 🟢 Bon")
    elif score >= 65:
        lines.append("État : 🟠 À surveiller")
    else:
        lines.append("État : 🔴 Attention")

    log_action("Diagnostic rapide", f"score={score}")
    popup("🔍 Diagnostic", "\n".join(lines), 620, 500)
...
def lancer_nettoyage():
    ...
    try:
        ...
        popup("🧹 Nettoyage", f"Impossible d'accéder au dossier temporaire.\n\n{exc}", 620, 380)
        return

    mb = total / (1024 ** 2)
    log_action("Nettoyage rapide", f"{deleted} éléments, {mb:.2f} Mo")
    popup(
        "🧹 Nettoyage",
        f"=== NETTOYAGE RAPIDE ===\n\n"
        f"Éléments supprimés : {deleted}\n"
        f"Espace libéré : {mb:.2f} Mo\n"
        f"Éléments non supprimés : {errors}",
        620,
        400
    )
...
def show_home():
    ...
    top.pack(fill="x", padx=30, pady=(28, 8))
    ctk.CTkLabel(top, text="Bienvenue sur PC Helper", font=("Arial", 30, "bold")).pack(anchor="w")
    ctk.CTkLabel(top, text="Un PC plus sain, plus simple à surveiller.", font=("Arial", 15)).pack(anchor="w", pady=(2, 0))
    ...
    creator.pack(anchor="e", pady=(0, 3))
    ...
    for i, (icon, title, key) in enumerate(stats):
        card = ctk.CTkFrame(cards, corner_radius=14, fg_color="transparent")
        card.grid(row=0, column=i, padx=7, sticky="nsew")
        ctk.CTkLabel(card, text=icon, font=("Arial", 24)).pack(anchor="w", padx=15, pady=(14, 2))
        ctk.CTkLabel(card, text=title, font=("Arial", 13, "bold")).pack(anchor="w", padx=15)
        stat_labels[key] = ctk.CTkLabel(card, text="--", font=("Arial", 20, "bold"))
        stat_labels[key].pack(anchor="w", padx=15, pady=(5, 15))

    lower = ctk.CTkFrame(content, fg_color="transparent")
    lower.pack(fill="both", expand=True, padx=30, pady=(2, 20))
    lower.grid_columnconfigure(0, weight=3)
    lower.grid_columnconfigure(1, weight=2)
    lower.grid_rowconfigure(0, weight=1)

    state_card = ctk.CTkFrame(lower, corner_radius=16, fg_color="transparent")
    state_card.grid(row=0, column=0, sticky="nsew", padx=(0, 10))
    ctk.CTkLabel(state_card, text="État général du PC", font=("Arial", 20, "bold")).pack(anchor="w", padx=22, pady=(22, 5))
    score_label = ctk.CTkLabel(state_card, text="", font=("Arial", 18, "bold"))
    score_label.pack(anchor="w", padx=22, pady=7)
    details = ctk.CTkLabel(state_card, text="Analysez votre PC pour obtenir une évaluation.", font=("Arial", 14), justify="left", anchor="w")
    details.pack(anchor="w", padx=22, pady=8)
    ctk.CTkButton(state_card, text="🔍 Lancer un diagnostic", height=46, command=lancer_diagnostic).pack(anchor="w", padx=22, pady=20)

    quick = ctk.CTkFrame(lower, corner_radius=16, fg_color="transparent")
    quick.grid(row=0, column=1, sticky="nsew", padx=(10, 0))
    ctk.CTkLabel(quick, text="Raccourcis rapides", font=("Arial", 20, "bold")).pack(anchor="w", padx=22, pady=(22, 12))
    for text, command in [
        ("🧹 Nettoyage rapide", lancer_nettoyage),
        ("💻 Informations système", afficher_infos_pc),
        ("📄 Rapport complet", open_full_report),
        ("📊 Surveillance", open_realtime_monitor),
    ]:
        ctk.CTkButton(quick, text=text, height=42, fg_color="transparent", border_width=1, command=command).pack(fill="x", padx=22, pady=6)

    footer = ctk.CTkLabel(content, text=f"© 2026 {APP_AUTHOR} — Créateur de PC Helper", font=("Arial", 11))
    ...
    def refresh_home():
        ...
        if score >= 85:
            score_label.configure(text=f"🟢 Tout est en bon état !  {score}/100")
        elif score >= 65:
            score_label.configure(text=f"🟠 Quelques points à surveiller  {score}/100")
        else:
            score_label.configure(text=f"🔴 Des vérifications sont recommandées  {score}/100")
        details.configure(text=f"CPU : {cpu:.0f} %    •    RAM : {ram.percent:.0f} %\nDisque : {go(disk.free):.1f} Go libres\nUptime : {fmt_duration(time.time() - psutil.boot_time())}")
        app.after(1500, refresh_home)

    refresh_home()


# category screens use the existing feature functions
def show_monitor_menu():
    clear_content()
    title = ctk.CTkLabel(content, text="🖥️ Surveillance", font=("Arial", 28, "bold"))
    title.pack(anchor="w", padx=30, pady=(28, 18))
    box = ctk.CTkFrame(content)
    box.pack(fill="both", expand=True, padx=30, pady=(0, 25))
    buttons = [
        ("📈 Tableau de bord", open_dashboard),
        ("📊 Moniteur en temps réel", open_realtime_monitor),
        ("💻 Informations du PC", afficher_infos_pc),
        ("🌡️ Températures", open_temperatures),
        ("🔋 Batterie", open_battery),
        ("🧩 Carte matériel", open_hardware),
        ("❤️ Score de santé", lambda: popup("❤️ Score de santé", f"Score indicatif : {calculate_health_score()}/100", 500, 330)),
    ]
    for i, (txt, cmd) in enumerate(buttons):
        box.grid_columnconfigure(i % 2, weight=1)
        ctk.CTkButton(box, text=txt, command=cmd, height=50).grid(row=i // 2, column=i % 2, padx=12, pady=12, sticky="ew")


def show_menu(title, groups):
    clear_content()
    ctk.CTkLabel(content, text=title, font=("Arial", 28, "bold")).pack(anchor="w", padx=30, pady=(28, 18))
    box = ctk.CTkScrollableFrame(content)
    box.pack(fill="both", expand=True, padx=30, pady=(0, 25))
    for group_title, buttons in groups:
        ctk.CTkLabel(box, text=group_title, font=("Arial", 18, "bold")).pack(anchor="w", pady=(12, 7))
        row = ctk.CTkFrame(box)
        row.pack(fill="x", pady=(0, 12))
        for i, (txt, cmd) in enumerate(buttons):
            row.grid_columnconfigure(i % 2, weight=1)
            ctk.CTkButton(row, text=txt, command=cmd, height=48).grid(row=i // 2, column=i % 2, padx=10, pady=8, sticky="ew")


nav_button("⌂  Accueil", show_home)
nav_button("🖥  Surveillance", show_monitor_menu)
nav_button("💾  Stockage", lambda: show_menu("💾 Stockage", [
...
nav_button("📶  Réseau", lambda: show_menu("🌐 Réseau", [
...
nav_button("🛠  Technicien", lambda: show_menu("🛠️ Technicien", [
    ("Outils Windows", [("🛠️ SFC / DISM", open_sfc_dism), ("💿 CHKDSK", open_chkdsk), ("🚀 Démarrage", open_startup), ("⚙️ Services", open_services), ("📊 Processus", open_processes), ("🧩 Pilotes", open_drivers), ("🔄 Mises à jour", open_windows_updates)])
]))
nav_button("🛡  Sécurité", lambda: show_menu("🔐 Sécurité", [
...
nav_button("📄  Rapports", lambda: show_menu("📄 Rapports", [
...
nav_button("🧰  Utilitaires", lambda: show_menu("😎 Utilitaires", [
...
ctk.CTkLabel(
    sidebar,
    text="Créé avec passion par Mathieu",
    font=("Arial", 11),
    wraplength=175,
    justify="left"
